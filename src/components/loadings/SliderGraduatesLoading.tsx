...
import GraduateLoading from './items/GraduateLoading'

export default function SliderGraduatesLoading({
    className
}: {
    className?: string
}) {
        ...
        <div className={cn('w-full relative', className)}>
            <div className='w-full h-full grid md:grid-cols-2 grid-cols-1 lg:gap-8 gap-4'>
                <div className='w-full h-full grid lg:grid-rows-3 sm:grid-rows-2 grid-rows-1 lg:gap-8 gap-4'>
                    <GraduateLoading />
                    <GraduateLoading className='lg:block hidden' />
                    <GraduateLoading className='sm:block hidden'/>
                </div>
